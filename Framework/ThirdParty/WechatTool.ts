...
export class WechatTool
{
    ...
    getAuthCodeUrl()
    {
        ...
        let url = "https://open.weixin.qq.com/connect/qrconnect?appid="+ GFCfg.third_cfg.wechat.app_id
        url+="&redirect_uri="+URLEncode.encode(GFCfg.third_cfg.wechat.redirect_uri)
    ...
    async getAccessInfo(auth_code:string):Promise<any>
    {
        ...
        let url="https://api.weixin.qq.com/sns/oauth2/access_token?appid="+GFCfg.third_cfg.wechat.app_id+"&secret="+GFCfg.third_cfg.wechat.app_key+"&code="+auth_code+"&grant_type=authorization_code"
        let rs = await GHttpTool.httpRequest(url)
        /*
        { 
