...
class AppleTool
{
    ...
    {
        GLog.info("begin onVerify============================"+environment,true)
        let url = this._verifyUrl
        // if(environment.toLowerCase()=="sandbox")
        // {
        //     url=this._sandboxVerifyUrl
        // }
        GLog.info("url============================"+url,true)
        let reqb = new RequestBody()
        reqb['receipt-data']=receipt
        //先验证生产环境
        var resb:ResponseBody = (await GHttpTool.httpPost(url,JSON.stringify(reqb))).body
        GLog.info("production end onVerify_Res============================status="+(resb?resb.status:"null"),true)
        //状态21007表示是沙盒环境
        if(resb&&resb.status==21007)
        {
            url=this._sandboxVerifyUrl
            resb = (await GHttpTool.httpPost(url,JSON.stringify(reqb))).body
            GLog.info("sandbox end onVerify_Res============================status="+(resb?resb.status:"null"),true)
        }
        GLog.info(resb)
        return resb
    }
    ...
    {
        let keyIdentifier="";
        for(var k in GFCfg.third_cfg.apple.keyIds)
        {
            keyIdentifier=k
            break
        }
        ...
        const path = GFCfg.third_cfg.apple.keyIds[keyIdentifier]
