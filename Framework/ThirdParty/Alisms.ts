import { GLog } from './../Logic/Log';
let SMSClient =require("@alicloud/sms-sdk")
import { GFCfg } from "../Config/FrameworkConfig";

export let GSmsTool:SMSTool=null
class SMSTool
{
    protected _is_init=false
    protected _sms_client=null
    init()
    {
        if(this._is_init)
        {
            return true
        }
        this._is_init = true
        this._sms_client = new SMSClient({accessKeyId:GFCfg.third_cfg.aliSms.accessKeyId,secretAccessKey:GFCfg.third_cfg.aliSms.secretAccessKey})
    }
    /**
     * 发送短信验证码
     * @param code 验证码
     * @param phone 手机号
     * @param callback 回调
     */
    sendSMS(code:number,phone:string):Promise<String>
    {
        return new Promise((resolve,reject)=>
        {
            this._sms_client.sendSMS({
                PhoneNumbers:phone,
                SignName:GFCfg.third_cfg.aliSms.signName,
                TemplateCode:GFCfg.third_cfg.aliSms.templateCode,
                TemplateParam:JSON.stringify({code:code})
            }).then((res)=>
            {
                if(res.Code=="OK")
                {
                    resolve(null)
                }
                else
                {
                    GLog.error("phone:"+phone+" code:"+res.Code)
                    resolve(res.Code)
                }
            },(err)=>
            {
                GLog.error(err)
                resolve(err)
            })
        })
    }
}
GSmsTool=new SMSTool()