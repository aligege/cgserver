...
export class QQTool
{
    ...
    getAuthCodeUrl(scope?:string,display?:string)
    {
        ...
        let redirect_uri = URLEncode.encode(GFCfg.third_cfg.qq.redirect_uri)
        //必须	申请QQ登录成功后，分配给应用的appid。
        let client_id = GFCfg.third_cfg.qq.app_id
        //必须	授权类型，此值固定为“code”。
    ...
    async getAccessToken(auth_code:string):Promise<string>
    {
        ...
        let client_secret = GFCfg.third_cfg.qq.app_key
        //必须	成功授权后的回调地址，必须是注册appid时填写的主域名下的地址，建议设置为网站首页或网站的用户中心
        let redirect_uri = URLEncode.encode(GFCfg.third_cfg.qq.redirect_uri)
    ...
    {
        let url = "https://graph.qq.com/user/get_user_info?access_token="+access_token+"&oauth_consumer_key="+GFCfg.third_cfg.qq.app_id+"&openid="+openid
        let rs = await GHttpTool.httpRequest(url)
        if(rs.body)
        {
            return rs.body
